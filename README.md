...
3. Ubicarse en la ruta de generacion del ejecutable de expressor. _(Debe estar ubicado en la raiz del proyecto (`...\expresso`) antes de ejecutar el siguiente comando)_. Ejecute:
...
4. Opcionalmete se puede ejecutar _(ubicado en la carpeta raíz del proyecto)_ el comando:

Gradle Wrapper
```bash
gradlew clean test
```
para ejecutar todos los test del proyecto. Aunque estos se van a ejecutar automaticamente con el `gradlew clean build` que se ejecuta al inicio para construir el proyecto.
